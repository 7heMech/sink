...
- Use Node.js 22 and Bun 1.4.0 (`package.json` is authoritative). The root package is the Nuxt app; `docs/` is the `@sink/docs` VitePress workspace package.
...
```bash
bun install           # also runs build:map, nuxt prepare, and hook setup
bun run dev           # Nuxt dev server on port 7465
bun run build         # production build with an 8 GB Node heap
bun run preview       # requires existing .output build artifacts
bun run dev:docs      # VitePress docs dev server
bun run build:docs    # production docs build
bun run preview:docs  # preview the docs build
bun run lint          # check only
bun run lint:fix      # modifies files
bun run types:check
bun run test --run                         # full Vitest run, not watch mode
bun run test --run tests/api/link.spec.ts  # one test file
bun run test --run -t 'creates new link'   # tests matching a name
```

- ESLint and TypeScript extend generated `.nuxt` files. If they are missing, run `bun run postinstall` (or `bun install`) before diagnosing config errors.
...
- Cloudflare bindings are declared in `wrangler.jsonc`: `DB`, `KV`, `ANALYTICS`, `AI`, `R2`, and `ASSETS`. Regenerate `worker-configuration.d.ts` with `bun run gen:types` after changing bindings.
...
```bash
bun run db:generate        # schema: server/database/schema.ts; output: drizzle/
bun run db:migrate:local   # apply migrations to local Wrangler D1
bun run db:migrate:remote  # mutates the configured remote D1 database
```
...
- `bun run deploy:pages` and `bun run deploy:worker` both run remote D1 migrations before publishing and assume build artifacts already exist. Treat them as production-mutating commands, not verification commands.
...
- Worker tests execute `.output/server/index.mjs` from `wrangler.jsonc`. Run `bun run build` after server changes before the final test run, or tests may exercise stale output.
...
- `bun install` regenerates ignored `public/world.json` via `build:map`.
- `bun run build` regenerates `public/sphere.bin` through its prebuild hook. `build:colo` and `build:testimonials` generate `public/colos.json` and `app/data/testimonials.json`; the testimonial task is network-dependent and randomizes order.
